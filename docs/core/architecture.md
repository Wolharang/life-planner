...
- **반복(recurrence, D35)**: `setAlarmClock()`은 1회성 → **발화 시 다음 occurrence를 예약**(AlarmReceiver가 다음
  날짜 계산 후 재예약). 네이티브 미러(§9-②)에 **반복 규칙 + 다음 유효시각**을 함께 저장해 BootReceiver/포그라운드
  재등록이 올바른 다음 발생을 잡게 한다.
- **단순 알림(plain reminders, D35)**: 실행 알람(FSI/setAlarmClock)과 **별개의 소프트 경로** — 다중 오프셋 평범한 로컬
  예약 알림(expo-notifications; FSI·풀스크린 아님, best-effort). `notifications` 인터페이스 뒤에 두고 **Task 삭제 시
  실행 알람과 함께 취소**. **S1 신뢰성 기준은 실행 알람에만** 적용(단순 알림 제외).
