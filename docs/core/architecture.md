...
> §0·§1·§2·§3·§10·§11)와 §5 코드트리의 `/calendar`·`/logs`·`/evaluation`·`/account`·`/core/data/sync.ts`(실제 위치) — 가 지금
...
- **로컬이 진실 소스(local-first, D2)**: 모든 읽기/쓰기는 로컬 먼저 → 오프라인에서도 완전 동작, 즉각 반응.
  ~~**로컬 저장소 = 전체앱은 Firestore 오프라인 지속성 캐시**~~ **← 폐기 (D51).** 실제: **AsyncStorage가 로컬
  원본**이고 Firestore는 **로그인 시 켜지는 미러**다 (계정 없이도 앱이 전부 동작해야 하므로 — D20/R4). *네이티브 알람 미러(SharedPreferences)는 진실 소스가 아닌 **파생 캐시**라 예외(§9).*
...
> **Repository 패턴이라는 핵심은 그대로**(features는 인터페이스만 호출; F0은 저장소를 교체한 게 아니라 **인터페이스
> 뒤에 Firestore 미러를 얹었다** — D51) —
...
| Repository 인터페이스 | ✅ | ✅ 동일 |
| 저장소 구현 | 로컬 전용(AsyncStorage) | ~~Firestore로 **교체**~~ → **AsyncStorage 유지 + Firestore 미러 추가** (D51; 인터페이스는 실제로 불변) |
...
| 3 **백업** | WorkManager 주기 워커(~15분)가 "놓친 알람"을 감지 → ~~즉시 발화~~ **미스로 기록만 하고 알람을 지운다** (2026-07-13 정정: **늦은 발화는 PRD §7.2가 금지한 스누즈다.** 잠금화면을 뒤늦게 점령하는 건 개입이 아니라 추궁이다. 그래서 발화하지 않고 R18 캐치업 그물에 넘긴다 — `AlarmBackupWorker.kt`) | 누락 캐치업 |
| 4 **자가 재등록** | BOOT_COMPLETED · **정확알람 권한변경**(2026-07-13에야 실제로 등록됨 — 그전엔 문서만 주장) · **시간대/시간 변경** · 앱 포그라운드 진입 시 **저장소에서 재예약**.<br>⚠ **시간대 변경 시 절대시각을 다시 걸면 안 된다**: 블록의 `start`는 **벽시계**다(§2.3 "절대시각 아님"). 미러가 **벽시계(날짜+분)** 를 함께 들고 있다가 **지금 시간대에서 순간을 다시 계산**한다. 단 **재확인(`#recheck`)은 예외** — 그건 "커밋 후 5분"이라는 **진짜 간격**이라 절대시각이 맞다 | 재부팅·권한·시간대·킬드 복구 |
