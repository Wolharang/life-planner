...
> **⚠ 갱신 (2026-07-11, full-app):** 아래 첫 두 항목은 **프로토타입 전용**이며 **D37/D38로 폐기**되었다.
> **반복 없음** — full-app의 TimeBlock은 날짜별 단일 인스턴스라 알람은 전부 **1회성**(발화 후 재예약 없음);
> "매일 헬스"는 추가 화면의 **여러 날짜 일괄 생성**으로 만든다(D37). **블록의 단순 알림 없음** — 블록의 알림은
> 실행 큐 하나뿐이고(D38·spec §3.9), 소프트 경로는 **중요일정 사전 알림(R3)** 에만 남는다. 소프트 경로는 전용
> 채널(**IMPORTANCE_DEFAULT · 무음 · 잠금화면 PRIVATE**)로 격리해 "뚫지 않음"을 코드가 아니라 **채널이 보장**한다.
- ~~**반복(recurrence, D35)**: `setAlarmClock()`은 1회성 → 발화 시 다음 occurrence를 예약…~~ **(폐기 — D37)**
- ~~**단순 알림(plain reminders, D35)**: 실행 알람과 별개의 소프트 경로를 **Task**에…~~ **(블록에서는 폐기 — D38;
  ImportantEvent의 R3 사전 알림으로만 존속)**
- **소리(R8)**: 실행 개입 소리는 **설정 `sound`(기본 false=햅틱만)**로 구동. 안드로이드 채널 소리는 생성 후 불변이므로
  **무음 알람 채널 + 명시적 톤 재생**(또는 소리/무음 이중 채널)로 default-off 보장.
...
4. **알람**: `executionAlarm` 블록 → 네이티브 스케줄 → OS → 정시 발화 → FSI → 실행 → done → Repository.
