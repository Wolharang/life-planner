...
- **클라이언트 생성 ID**: 각 레코드 `id`는 앱이 만든 **랜덤 ID**(`src/core/data/id.ts`) → 서버 없이(오프라인) 즉시
  ...
  서로 무관한 두 레코드를 조용히 합쳐버린다. **충돌 판정(구현된 실제 규칙, 2026-07-13 정정)**: `serverTimestamp()`는
  **코드 어디에도 쓰이지 않는다**. Firestore 자체의 **"서버에 나중에 도달한 쓰기가 이긴다"** 가 곧 판정이다 — §6이
  이미 그 한계("편집 시각이 아니라 도달 순서")를 알고 받아들였다. 클라 `updatedAt`은 **충돌 판정이 아니라**
  재결합 시 "누가 상대에게 없는 것을 갖고 있는가"를 가리는 데만 쓴다(D53).
...
| ~~**ImportantEvent**~~ | **폐기 (D67)** — `alert:"none"`인 TimeBlock으로 흡수 | | | |
...
- **⚠ 구현 현황(2026-07-13) — 이 엔티티는 아직 없다.** `/users/{uid}` 문서는 **한 번도 쓰이지 않는다**; sync는 네 개의
  하위 컬렉션만 만진다. 설정은 **로컬 전용**(`lp.settings.v1`, `defaultLeadMinutes` 하나뿐 — `theme` 없음)이고,
  신원은 **이메일**이다(`loginId` 아님 — D52로 Google 로그인까지 들어왔다). 설정 동기화는 **미구현**이다.

### 2.2 ~~ImportantEvent~~ — **폐기됨 (D67, 2026-07-14)**

> **⚠ 이 엔티티는 더 이상 존재하지 않는다.** "중요 일정"은 처음부터 **`alert: "none"`인 TimeBlock**이었다.
> 둘로 나눠두니 사용자가 자기 삶과 무관한 질문(*"이건 일정인가 블록인가?"*)에 답해야 했고, 그 대답이 벌을 받았다 —
> **블록으로 넣으면 캘린더에서 사라졌다.** 절반의 약속을 숨기는 캘린더는 없는 것만 못하다.
>
> **`lp.events.v1`은 blockRepository가 1회 마이그레이션 후 폐기한다** (리드타임 있으면 `soft`, 없으면 `none`;
> id·색·메모 보존). 아래 표는 **역사 기록**이며, 필드는 전부 TimeBlock(§2.3)으로 흡수됐다(`color`, `memo`).
> 동기화의 `events` 컬렉션도 은퇴했다.

#### (역사) ImportantEvent — 중요 일정 (계층1)
| 필드 | 타입 | 필수 | 생성/출처 | 비고 |
|---|---|---|---|---|
...
- **저장**: `/users/{uid}/blocks/{id}` (문서는 오래 `timeblocks`라고 적었지만 **구현·실데이터는 `blocks`** — 2026-07-13
  정정; 컬렉션 이름은 규칙에서 와일드카드라 강제되지 않는다) + 로컬 미러. **현재 로컬 구현(2026-07-11)**: AsyncStorage `lp.blocks.v1`
...
```
        ...
        ┌──────────────┬─────────┼──────────────┬──────────────┐
        ▼              ▼         (소유: uid)      ▼              ▼
  TimeBlock (하나의 단위 — D67)                 Expense        MealEntry
...
- ~~**ImportantEvent ↔ TimeBlock**~~ **(D67 폐기)** — 둘은 애초에 같은 것이었다. **단위는 TimeBlock 하나**이고,
  **`alert` 단계가 곧 그것이 무엇인지**를 말한다: `none`=시간만 차지(평가 안 함) · `soft`=중요 · `execution`=레버
...
> **저장 모델 (F0 구현, 2026-07-13 · D51).** **AsyncStorage가 로컬 원본이고, Firestore는 로그인 시 켜지는 미러**다.
> D34의 "Firestore 오프라인 저장소 = 유일한 로컬 저장소"는 **D20/R4(계정 없이도 앱 전체 동작)** 와 양립할 수 없어
> 그 조항만 폐기됐다 — 계정이 없으면 `uid`가 없고, `uid`가 없으면 **쓸 문서 경로 자체가 없다**. 로그아웃 상태의 앱은
> F0 이전과 완전히 동일하며, 그래서 **R11(비행기모드 동일 동작)이 공짜로 성립**한다.
> **깨면 데이터가 죽는 순서 하나:** 이관(P-c)은 **구독보다 먼저 push**한다. 리스너는 스냅샷을 로컬에 기록하므로,
> 구독이 먼저면 **새 계정의 빈 클라우드가 계획으로 가득 찬 폰을 덮어써 지운다.**
> **동기화하지 않는 것:** `lp.outcomes/fires/missed/latencies`(§2.7) — 이 기기가 레버로 **실제 무엇을 했는지의 증거**
> (S1–S5)이지 공유 상태가 아니다. 두 기기의 발화 로그를 합치면 자가실험이 오염된다.
- **ID**: 클라이언트 UUID(오프라인 생성). **충돌 판정(실제)**: **서버에 나중에 도달한 쓰기가 이긴다** — Firestore의
  last-write-wins 그 자체다. `serverTimestamp()`를 필드로 쓰지는 **않는다**(코드에 없다; 2026-07-13 정정).
  클라이언트 `updatedAt`은 시계 편차 때문에 **충돌 판정에는 쓰지 않고**, 재결합 시 "누가 상대에게 없는 것을
  갖고 있는가"를 가리는 데만 쓴다(D53).
...
- **삭제**: **소프트 삭제**(`deletedAt` 툼스톤) → 기기 간 전파(하드 삭제는 되살아날 위험). **툼스톤은 Firestore에만
  존재한다** — 로컬은 그냥 지운다(엔티티 타입에 `deletedAt` 필드가 없다). 그리고 **툼스톤은 최종이다**: 되살리는
  업데이트를 **보안 규칙이 서버에서 거부**한다(D54). **읽기는 `deletedAt==null`
...
| 소프트 삭제(툼스톤, **되돌릴 수 없음** — 서버 규칙이 강제) + **도달 순서 기준** last-write-wins | 다기기 정합성 | D53, D54, §6 |
